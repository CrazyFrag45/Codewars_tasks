...
He isn't fond of seeing the Eiffel tower 40 times. He tells them that he
will only sit during the session if they show the same motive at most N times.
...
def delete_nth(order,max_e):
    ...
    for i in range(len(order)):
        if order[i] not in new_arr:
            new_arr.append(order[i])
            count_arr[order[i]] = 1
        elif count_arr[order[i]] < max_e:
            new_arr.append(order[i])
            count_arr[order[i]] += 1
    return new_arr
...
print(delete_nth2( [1,1,3,3,7,2,2,2,2], 3 ))
